...
func (p Prediction) SimpleMap() map[string]int {
	return map[string]int{
		"Drawings": int(math.Round(float64(p.Drawings * 100))),
		"Hentai":   int(math.Round(float64(p.Hentai * 100))),
		"Neutral":  int(math.Round(float64(p.Neutral * 100))),
		"Porn":     int(math.Round(float64(p.Porn * 100))),
		"Sexy":     int(math.Round(float64(p.Sexy * 100))),
	}
}
